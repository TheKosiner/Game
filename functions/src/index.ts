...
function buildVerificationEmail(email: string, link: string, lang: string): string {
  const isPl = lang === 'pl';
  const title    = isPl ? 'WERYFIKACJA KONTA' : 'ACCOUNT VERIFICATION';
  const subtitle = isPl ? 'Weryfikujesz adres e-mail:' : 'Confirming your email address:';
  const btnText  = isPl ? '⚔ ZWERYFIKUJ EMAIL' : '⚔ VERIFY EMAIL';
  const note1    = isPl ? 'Link jest ważny przez 24 godziny.' : 'This link expires in 24 hours.';
  const note2    = isPl
    ? 'Jeśli nie zakładałeś konta w Glitch Soul, zignoruj tę wiadomość.'
    : 'If you did not create a Glitch Soul account, please ignore this email.';
  const spamNote = isPl
    ? '⚠ Sprawdź folder SPAM jeśli ta wiadomość tam trafiła'
    : '⚠ Check your SPAM folder if you cannot find this email';
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card {
    ...
    padding: 36px 28px;
    text-align: center;
    box-shadow: 0 0 40px rgba(251,191,36,0.06), inset 0 0 60px rgba(0,0,0,0.4);
  }
  .corner {
    position: absolute;
    width: 12px; height: 12px;
  }
  .logo { width: 72px; height: 72px; margin-bottom: 20px; image-rendering: pixelated; }
  .divider { border: none; border-top: 1px solid rgba(251,191,36,0.2); margin: 20px 0; }
  .title {
    font-size: 14px;
    color: #fbbf24;
    letter-spacing: 2px;
  ...
  .subtitle { font-size: 8px; color: #64748b; margin-bottom: 16px; }
  .email-box {
  ...
  .btn {
    ...
    background: linear-gradient(135deg, rgba(251,191,36,0.18) 0%, rgba(0,0,0,0.9) 100%);
  ...
  .note { font-size: 7px; color: #475569; line-height: 2; margin-bottom: 8px; }
  .spam-note { font-size: 7px; color: #b45309; margin-top: 12px; }
  .footer { text-align: center; font-size: 7px; color: #1e293b; margin-top: 20px; }
</style>
</head>
<body>
<div class="wrapper">
  <div class="card">
    <img src="https://glitchsoul.com/favicon.png" alt="Glitch Soul" class="logo" />
    <p class="title">${title}</p>
    <p class="subtitle">${subtitle}</p>
    <div class="email-box">${email}</div>
    <hr class="divider" />
    <a href="${link}" class="btn">${btnText}</a>
    <p class="note">${note1}<br />${note2}</p>
    <p class="spam-note">${spamNote}</p>
  </div>
  <p class="footer">© 2025 Glitch Soul · glitchsoul.com</p>
</div>
</body>
</html>`;
}
...
export const sendVerificationEmail = functions
  ...
  .https.onCall(async (data: { lang?: string }, context) => {
    if (!context.auth) {
    ...
    const html = buildVerificationEmail(email, verificationLink, lang);

    const resend = new Resend(apiKey);
    const subject = lang === 'pl'
      ? 'Weryfikacja konta — Glitch Soul'
      : 'Verify your account — Glitch Soul';

    const { error } = await resend.emails.send({
      from: 'Glitch Soul <noreply@glitchsoul.com>',
      to: email,
      subject,
      html,
    });
